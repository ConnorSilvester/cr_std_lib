...
static u8 cr_std_logger_current_log_level = CR_STD_LOGGER_LOG_LEVEL_ALL;
...
void cr_std_logger_init() {
    if (!g_log_arena) {
        g_log_arena = cr_std_arena_new(CR_STD_MB);
    }

    if (!g_log_history) {
        g_log_history = cr_std_vector_new(g_log_arena);
    }
}
...
void cr_std_logger_record_log(int log_type, const char *log_message) {
    if (cr_std_logger_record_log_flag) {
        ...
        case CR_STD_LOGGER_LOG_TYPE_INFO:
            log = cr_std_string_newf(g_log_arena, "[%s] [INFO] %s\n", time_str, log_message);
            break;
        case CR_STD_LOGGER_LOG_TYPE_WARNING:
            log = cr_std_string_newf(g_log_arena, "[%s] \033[33m[WARNING]\033[0m %s\n", time_str,
                                     log_message);
            break;
        case CR_STD_LOGGER_LOG_TYPE_ERROR:
            log = cr_std_string_newf(g_log_arena, "[%s] \033[31m[ERROR]\033[0m %s\n", time_str,
                                     log_message);
            break;
        default:
            break;
        ...
        if (log) {
            cr_std_vector_push_back(g_log_arena, g_log_history, log);
        }
    }
}
...
void cr_std_logger_record_log_custom(const char *type, int color_code, const char *log_message) {
    if (cr_std_logger_record_log_flag) {
        ...
        String *log = cr_std_string_newf(g_log_arena, "[%s] \033[%dm[%s]\033[0m %s\n", time_str,
                                         color_code, type, log_message);
        if (log) {
            cr_std_vector_push_back(g_log_arena, g_log_history, log);
        }
    }
...
void cr_std_logger_print_history() {
    ...
    for (int i = 0; i < g_log_history->size; i++) {
        String *current_log = cr_std_vector_get_at(g_log_history, String, i);
        printf("%s", current_log->c_str);
    }
}

void cr_std_logger_clear_history() {
    cr_std_arena_free(&g_log_arena);
    g_log_arena = NULL;
    g_log_history = NULL;
}
...
b8 cr_std_logger_write_history_to_file(const char *filepath) {
    ...
    if (g_log_history->size > 0) {
        ...
        for (size_t i = 0; i < g_log_history->size; i++) {
            String *current_log = cr_std_vector_get_at(g_log_history, String, i);
